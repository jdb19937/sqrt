...
static sidus_genus_t sidus_genus_ex_nomine(const char *n)
{
    ...
    if (strcmp(n, "nanum_album") == 0)
        return SIDUS_NANUM_ALBUM;
    if (strcmp(n, "sequentia") == 0)
        return SIDUS_SEQUENTIA;
    if (strcmp(n, "gigas_rubrum") == 0)
        return SIDUS_GIGAS_RUBRUM;
    if (strcmp(n, "supergigas") == 0)
        return SIDUS_SUPERGIGAS;
    if (strcmp(n, "neutronium") == 0)
        return SIDUS_NEUTRONIUM;
    if (strcmp(n, "crystallinum") == 0)
        return SIDUS_CRYSTALLINUM;
    if (strcmp(n, "magnetar") == 0)
        return SIDUS_MAGNETAR;
    if (strcmp(n, "galaxia") == 0)
        return SIDUS_GALAXIA;
    if (strcmp(n, "vagans") == 0)
        return SIDUS_VAGANS;
    return SIDUS_SEQUENTIA;
}

...
static double strepitus_bilis(double fx, double fy)
{
    ...
    double tx = fx - ix;  double ty = fy - iy;
...
planeta_perceptus_t planeta_perceptus_ex_ison(const char *ison)
{
    ...
    if (asp_s) { p.aspectus = planeta_aspectus_ex_ison(asp_s); free(asp_s); }

    char *co_s = ison_da_crudum(ison, "coaspectus");
    if (co_s)  { p.coaspectus = planeta_aspectus_ex_ison(co_s); free(co_s); }
...
{
    if (!p) return;
    if (p->acuitas < 0.001 && p->detallum < 0.001
     && p->granum  < 0.001 && p->aberratio < 0.001) return;

...
void reddere_crystallinum(
    ...
    sidus_semen_g   = (unsigned int)(s->pro.temperatura * 137);
    ...
    for (int i = 0; i < n_fil; i++) {
        ...
        double ang = sidus_alea_f() * DUO_PI;
        double dx  = cos(ang), dy = sin(ang);

        /* longitudo variat */
        double len = 3.0 + sidus_alea_f() * (8.0 + luciditas * 8.0);
        if (len > 26.0)
            len = 26.0;

        /* color spectrale — unum ex 6 */
        color_t fc = spectra[sidus_alea() % 6];
...
void crystallinum_ex_ison(crystallinum_t *s, const char *ison)
{
    ...
    s->pro.temperatura = ison_da_f(ison, "sidulum.temperatura", 15000);
}
